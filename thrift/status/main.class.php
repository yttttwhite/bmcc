...
include_once(dirname(__FILE__)."/StatusService.php");

class thrift_status_main{
	public function __call($name,$arguments){

		$config = SConfig::getConfig(ROOT_CONFIG."/thrift.conf","status");
		$socket = new TSocket($config->host, $config->port);
		$transport = new TBufferedTransport($socket, 1024, 1024);
		$protocol = new TBinaryProtocol($transport);
		$transport->open();
		$client = new StatusServiceClient($protocol);  
		try{
