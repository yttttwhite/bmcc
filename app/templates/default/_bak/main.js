(function(win, headVar){
    ...
    api.log = function(s){
        if (parseInt(api._log_flag)) {
            ...
            var logUrl = "https://" + api._log + "/stat.log.test?" + p + "&rand=" + Math.random();
    ...
    api.show_js = function(_js, _js2, _sn, aid){
        ...
        api.js(_js + "?" + Math.ceil(Math.random() * 100000000), function(){
            ...
            if (typeof _js2 != "undefined" && _js2 != "undefined" && _js2 != "") {
                ...
                iframe.setAttribute("src", "https://" + api._js + "/stat.show?bidder={{{$bidder}}}&aid=" + aid);
    ...
    api.run = function(sn){
        ...
        if (!span) {
            ...
            if (type != "") {
				...
				//alert(ad.data.view_type);
				//alert(ad.data.staff[0].type);
                    ...
                    if (ad.data.staff[0].type == 5 || ad.data.staff[0].type == "AD_REDIRECT") {
                        ...
                        if (ad.data.staff[0].type == 2 || ad.data.staff[0].type == "AD_FLASH") {
                            ...
                            if (ad.data.staff[0].type == 8 || ad.data.staff[0].type == "AD_JS") {
                                if (ad.data.staff[0].text.indexOf("https") == -1) {
                                    ad.data.staff[0].text = "https://" + ad.data.staff[0].text
                                }
    ...
    api.genFuChuangWrapper = function(ad, staff){
		...
		var statJS = "";
		var clickJs = "";
		if(mim_isset(staff.landing_page) && (ad.data.staff[0].type == "AD_IMAGE" || ad.data.staff[0].type == "AD_FLASH")){
			if (staff.landing_page.indexOf("https") == -1) {staff.landing_page = "https://" + staff.landing_page}
			statJS = " window.open('https://" + api._redirect + "/stat.log.redirect?url=" + escape(staff.landing_page) + "');";
		...
		closeImgSrc = "https://" + api._js + "/assets_admin/img/close-white.png";
        if (staff.width > 1 || staff.height > 1) {
			var closeImg = '<img onclick="'+closeJs+'" src="'+closeImgSrc+'" style="width:12px; position: relative; z-index: 10000; cursor: pointer;"/>';
            ...
            var closeDiv = '<div style="line-height: 100%;padding:5px; background:rgba(0,0,0,0.4); border-radius: 50%; font-size:0px;">'+closeImg+clickDiv+'</div>';
        }else {
            var closeDiv = "<span>×</span>"
        }
    ...
    api.genDitongWrapper = function(ad, staff){
		...
		if(ad.data.staff[0].type != "AD_IMAGE" && ad.data.staff[0].type != "AD_FLASH"){
			wrap.style.height = staff.height + "px";
		}
		console.log(ad.data.staff[0].type);
		...
		closeImgSrc = "https://" + api._js + "/assets_admin/img/close-white.png";
		if (staff.width > 1 || staff.height > 1) {
			var closeImg = '<img onclick="'+closeJs+'" src="'+closeImgSrc+'" style="width:25px; position: relative; z-index: 10000; cursor: pointer;"/>';
			var clickDiv = '<div onclick="'+clickJs+statJS+'" style="position: absolute; z-index:0; width: 100%; height: 100%; top: 0; left: 0; cursor: pointer;"></div>'
		    var closeDiv = '<div style="line-height: 100%; padding:20px; background:rgba(0,0,0,0.4); border-radius: 50%; font-size:0px;">'+closeImg+clickDiv+'</div>';
		}else {
		    var closeDiv = "<span>×</span>"
		}
    ...
    api.genNormal = function(ad, staff){
        ...
        wrap_content.id = ad.sn + "_content";
		wrap_content.style.margin = "auto";
		if(ad.data.view_type == 1 && (api.isMobile() || staff.width>10000)){
			if(staff.width > 10000 && staff.width < 20000){
				wrap_content.style.width = staff.width - 10000 + "px";
				if(staff.height > 10000){
					wrap_content.style.height = staff.height - 10000 + "px";
				}else{
					wrap_content.style.height = staff.height + "px";
				}
				console.log('xxx');
			...
			wrap_content.style.position = "absolute";
			wrap_content.style.zIndex= "999999";
		}else{
			wrap_content.style.width = staff.width + "px";
        	wrap_content.style.height = staff.height + "px";
			console.log('123');
		...
		if (api.isMobile()) {
			...
			closeImgSrc = "https://" + api._js + "/assets_admin/img/close-white.png";
			if (staff.width > 1 || staff.height > 1) {
				var closeImg = '<img onclick="' + closeJs + '" src="' + closeImgSrc + '" style="width:15px; position: relative; z-index: 10000; cursor: pointer;"/>';
				var clickDiv = '<div onclick="' + clickJs + statJS + '" style="position: absolute; z-index:0; width: 100%; height: 100%; top: 0; left: 0; cursor: pointer;"></div>'
				var closeDiv = '<div style="line-height: 100%; padding:10px; background:rgba(0,0,0,0.4); border-radius: 50%; font-size:0px;">' + closeImg + clickDiv + '</div>';
			}else {
				var closeDiv = "<span>×</span>"
			}
			var c_content = document.createElement("div");
    ...
    api.renderMobileView = function(ad, staff){
        ...
        if (staff.landing_page.indexOf("https") == -1) {
            staff.landing_page = "https://" + staff.landing_page
        }
    ...
    api.renderMobileViewIframe = function(sn, addr){
        ...
        var divObject = document.getElementById(sn + "_div");
        ...
        if (width == 0) {
            ...
            divObject.style.height = "100%"
        }
        else {
            iframe.width = "0";
            iframe.height = "0"
            divObject.style.width = "0";
    ...
    api.renderMobileApp = function(ad, staff){
        ...
        api.load("https://cdn.tanx.com/t/tanxmobile/mraid.js", function(){
    ...
    api.renderPic = function(ad, staff, wrap){
        var p = document.createElement("div");
		if (ad.data.view_type != 32 && ad.data.view_type != 1) {
        ...
        if (typeof staff.landing_page != "undefined" && staff.landing_page != "undefined") {
            ...
            href = " href='https://" + api._redirect + "/stat.log.redirect?url=" + escape(staff.landing_page) + "' "
        }
		if(ad.data.view_type == 32 || ad.data.view_type == 1){
			p.innerHTML = "<a onclick='javascript:BCMain.click_js(\"" + click_js + '","' + escape(ad.data.third_click_js) + '","' + ad.sn + '","' + ad.data.aid + "\");' style='' " + href + " target='_blank'><img  width='100%'  border='0' src='" + staff.addr + "'/></a>";
		}else{
			p.innerHTML = "<a onclick='javascript:BCMain.click_js(\"" + click_js + '","' + escape(ad.data.third_click_js) + '","' + ad.sn + '","' + ad.data.aid + "\");' style='' " + href + " target='_blank'><img  width='100%'  height='" + staff.height + "' border='0' src='" + staff.addr + "'/></a>";
		}
        api.show_js(show_js, escape(ad.data.third_show_js), ad.sn, ad.data.aid);
        if (wrap != "") {
            wrap.childNodes[0].appendChild(p)
    ...
    api.renderSwf = function(ad, staff, wrap){
        ...
        if (typeof staff.landing_page != "undefined" && staff.landing_page != "undefined") {
            ...
            var href = "https://" + api._redirect + "/stat.log.redirect?url=" + escape(staff.landing_page);
            ...
            h += '<img width="100%" src="https://' + api._js + '/assets/images/null.gif" style="border:0px" height="100%" />';
            h += "</object>";
            h += "</a>   "
        }
    ...
    api.show = function(){
        ...
        api.write("<iframe id='" + sn + "' style='margin:0px;padding:0px;height:0px;width:0px;float:left;' frameborder=0 scrolling='no' marginwidth='0' marginheight='0'></iframe>");
