...
class Response {
  ...
  public function __construct($vals=null) {
    ...
    if (is_array($vals)) {
      ...
      if (isset($vals['adspace'])) {
        $this->adspace = $vals['adspace'];
      }
      if (isset($vals['day'])) {
        $this->day = $vals['day'];
      }
  ...
  public function read($input)
  {
    ...
    while (true)
    {
      ...
      switch ($fid)
      {
        ...
        case 15:
          if ($ftype == TType::STRING) {
            $xfer += $input->readString($this->adspace);
          } else {
            $xfer += $input->skip($ftype);
          }
          break;
  ...
  public function write($output) {
    ...
    if ($this->adspace !== null) {
      $xfer += $output->writeFieldBegin('adspace', TType::STRING, 15);
      $xfer += $output->writeString($this->adspace);
      $xfer += $output->writeFieldEnd();
    }
    if ($this->day !== null) {
      $xfer += $output->writeFieldBegin('day', TType::STRING, 16);
      $xfer += $output->writeString($this->day);
      $xfer += $output->writeFieldEnd();
    }
    if ($this->uid !== null) {
      $xfer += $output->writeFieldBegin('uid', TType::STRING, 17);
