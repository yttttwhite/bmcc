...
<div class="accordion" style="margin-top:0;">
	...
	<div class="snav" style="display:block;" {if $nav=="plan"}current="active"{/if}>
		<ul>
			<li {if $nav_sub==1}class="sel"{/if}><a href="/admin.shenhe.plan?type=1"><b>待审核</b></a></li>
			<li {if $nav_sub==3}class="sel"{/if}><a href="/admin.shenhe.plan?type=3">末通过</a></li>
			<li {if $nav_sub==2}class="sel"{/if}><a href="/admin.shenhe.plan?type=2">已通过</a></li>
		</ul>
	</div>
	...
	<div class="snav" style="display:block;" {if $nav=="stuff"}current="active"{/if}>
		<ul>
			<li {if $nav_sub==1}class="sel"{/if}><a href="/admin.shenhe.stuff?type=1"><b>待审核</b></a></li>
			<li {if $nav_sub==3}class="sel"{/if}><a href="/admin.shenhe.stuff?type=3">末通过</a></li>
			<li {if $nav_sub==2}class="sel"{/if}><a href="/admin.shenhe.stuff?type=2">已通过</a></li>
		</ul>
	</div>
	...
	{if(user_api::auth("admin"))}
		...
		<div class="snav" style="display:block;" {if $nav=="user"}current="active"{/if}>
			<ul>
				<li {if $nav_sub=="list"}class="sel"{/if}><a href="/admin.user.list">广告主列表</a></li>
				<li {if $nav_sub=="add"}class="sel"{/if}><a href="/admin.user.add">添加</a></li>
			</ul>
		</div>
	{/if}
</div>