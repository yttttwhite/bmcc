...
{if(user_api::auth("media") && $config['dsp']['media'])}
	<a href="/media?nav=2"><h3 class="bort {if $nav=="media"}active{/if}">媒体来源</h3></a>

	<a href="/media.channel?nav=2"><h3 class="bort {if $nav=="channel"}active{/if}">频道专题</h3></a>

    <a href="/media.tag?nav=2"> <h3 class="bort {if $nav=="tag"}active{/if}">广告位分类</h3></a>

    <a href="/media.position?nav=2"> <h3 class="bort {if $nav=="position"}active{/if}">广告位置</h3></a>
{/if}
    <a href="/media.schedule?nav=2" > <h3 class="bort {if $nav=="schedule"}active{/if}">广告位排期</h3></a>
