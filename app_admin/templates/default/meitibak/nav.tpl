...
<div class="accordion" style="margin-top:0;">
	...
	<div class="snav" style="display:block;" {if $nav=="slot"}current="active"{/if}>
		<ul>
			<li {if $nav=="slot" && $nav_sub=="list"}class="sel"{/if}><a href="/baichuan_advertisement_manage/media.slot">广告位列表</a></li>
			<li {if $nav=="slot" && $nav_sub=="add"}class="sel"{/if}><a href="/baichuan_advertisement_manage/media.slot.add">新增广告位</a></li>
		</ul>
	</div>
