...
<link type="text/css" rel="stylesheet" href="/baichuan_advertisement_manage/assets_admin/v5/css/style_v5.css" />
<link type="text/css" rel="stylesheet" href="/baichuan_advertisement_manage/assets_admin/css/jquery-ui-1.10.3.custom.css" />

<!-- 前端框架及插件 -->
<link href="/baichuan_advertisement_manage/assets_admin/v5/css/bootstrap.min.css" rel="stylesheet">
<link href="/baichuan_advertisement_manage/assets_admin/v5/plugins/font-awesome/css/font-awesome.min.css" rel="stylesheet">
<link href="/baichuan_advertisement_manage/assets_admin/v5/plugins/iCheck/skins/all.css" rel="stylesheet">
...
<!--<script type="text/javascript" src='/assets_admin/js/echarts-2.0.0/build/echarts-plain-map.js'></script>-->
...
define(["jquery", "../jquery.validate"], factory )}else{ factory( jQuery )}
    ...
    equalTo: "你的输入不相同",
    extension: "请输入有效的后缀",
    maxlength: $.validator.format("最多可以输入 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>0} 个字符"),
    minlength: $.validator.format("最少要输入 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>0} 个字符"),
    rangelength: $.validator.format("请输入长度在 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>0} 到 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>1} 之间的字符串"),
    range: $.validator.format("请输入范围在 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>0} 到 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>1} 之间的数值"),
    max: $.validator.format("请输入不大于 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>0} 的数值"),
    min: $.validator.format("请输入不小于 <?php echo htmlspecialchars('{', ENT_QUOTES); ?>0} 的数值")});
}));
</script>
<!--插件：jquery-layer-->
