...
class getid3_write_id3v2
{
			...
			switch ($frame_name) {
				...
				case 'COMR':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
						...
						$this->errors[] = 'Invalid Contact URL in '.$frame_name.' ('.$source_data_array['contacturl'].') (allowed schemes: https, https, ftp, mailto)';
	...
	public function IsValidURL($url, $allowUserPass=false) {
		...
		if ($parts = $this->safe_parse_url($url)) {
			if (($parts['scheme'] != 'https') && ($parts['scheme'] != 'https') && ($parts['scheme'] != 'ftp') && ($parts['scheme'] != 'gopher')) {
				return false;
