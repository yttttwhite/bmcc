...
jQuery.extend({
		...
		var onreadystatechange = function(isTimeout){
			...
			if ( !requestDone && xhr && (xhr.readyState == 4 || isTimeout == "timeout") ) {
				...
				status = isTimeout == "timeout" && "timeout" ||
					!jQuery.httpsSuccess( xhr ) && "error" ||
					s.ifModified && jQuery.httpsNotModified( xhr, s.url ) && "notmodified" ||
					"success";
				...
				if ( status == "success" ) {
					...
					try {
						...
						data = jQuery.httpsData( xhr, s.dataType, s.dataFilter );
					} catch(e) {
						status = "parsererror";
					}
				}
	...
	httpsData: function( xhr, type, filter ) {
