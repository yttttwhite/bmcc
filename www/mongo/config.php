 ...
 * @package rockmongo
 */
$config = SConfig::getConfig(ROOT_CONFIG."/mongo.conf","dpc_mongo");
$MONGO = array();
...
* 
* @see more details at https://rockmongo.com/wiki/configuration?lang=en_us
*/
$MONGO["servers"][$i]["mongo_name"] = $config->host;//"Localhost";//mongo server name
//$MONGO["servers"][$i]["mongo_sock"] = "/var/run/mongo.sock";//mongo socket path (instead of host and port)
$MONGO["servers"][$i]["mongo_host"] = $config->host;//"127.0.0.1";//mongo host
$MONGO["servers"][$i]["mongo_port"] = $config->port;//"27017";//mongo port
$MONGO["servers"][$i]["mongo_timeout"] = 0;//mongo connection timeout
$MONGO["servers"][$i]["mongo_db"] = $config->database;//default mongo db to connect, works only if mongo_auth=false
$MONGO["servers"][$i]["mongo_user"] = $config->user;//mongo authentication user name, works only if mongo_auth=false
$MONGO["servers"][$i]["mongo_pass"] = $config->password;//mongo authentication password, works only if mongo_auth=false
