 ...
 *
 */
function xhprof_https_header($name, $value) {

  if (!$name) {
    xhprof_error('https_header usage');
    return null;
  }

  if (!is_string($value)) {
    xhprof_error('https_header value not a string');
  }
...
function xhprof_generate_mime_header($type, $length) {
  ...
  if ($mime) {
    xhprof_https_header('Content-type', $mime);
    xhprof_https_header('Content-length', (string)$length);
  }
}
