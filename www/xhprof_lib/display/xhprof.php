...
function profiler_report ($url_params,
  ...
  $top_link_query_string = "$base_path/?" . https_build_query($base_url_params);
  ...
  if ($diff_mode) {
    ...
    $run1_link = xhprof_render_link('View Run #' . $run1,
                           "$base_path/?" .
                           https_build_query(xhprof_array_set($base_url_params,
                                                      'run',
                                                      $run1)));
  ...
  if ($diff_mode) {
    ...
    $links [] = xhprof_render_link('Invert ' . $diff_text . ' Report',
                           "$base_path/?".
                           https_build_query($inverted_params));
  }
...
function print_function_info($url_params, $info, $sort, $run1, $run2) {
  ...
  $href = "$base_path/?" .
           https_build_query(xhprof_array_set($url_params,
                                             'symbol', $info["fn"]));
...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit) {
  ...
  foreach ($stats as $stat) {
    ...
    if (array_key_exists($stat, $sortable_columns)) {
      $href = "$base_path/?"
              . https_build_query(xhprof_array_set($url_params, 'sort', $stat));
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2) {
  ...
  print("<center><br><h3>" .
        xhprof_render_link($callgraph_report_title,
                    "$base_path/callgraph.php" . "?" . https_build_query($url_params))
        . "</h3></center>");
...
function symbol_report($url_params,
  ...
  if ($diff_mode) {
    ...
    $base_url_params = xhprof_array_unset(xhprof_array_unset($url_params,
                                          ...
                                          'run2');
    $href1 = "$base_path?"
      . https_build_query(xhprof_array_set($base_url_params, 'run', $run1));
    $href2 = "$base_path?"
      . https_build_query(xhprof_array_set($base_url_params, 'run', $run2));
  ...
  $callgraph_href = "$base_path/callgraph.php?"
    . https_build_query(xhprof_array_set($url_params, 'func', $rep_symbol));
