...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
		...
		foreach ($_phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
			...
			} else if ( preg_match('/^(https:|https:|ftp:|mailto:)/', $url) ) {
				// URL
				// $url = $url;
	 ...
	 *
	 * The hyperlink can be to a https, ftp, mail, internal sheet (not yet), or external
	 * directory url.
	 *
	 * Returns  0 : normal termination
	 *		 -2 : row or column out of range
	 ...
	 * to be written. These are either, Web (https, ftp, mailto), Internal
	...
	/**
	 * Used to write https, ftp and mailto hyperlinks.
