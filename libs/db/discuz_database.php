...
class discuz_database_safecheck {
	...
	public static function checkquery($sql) {
		if (self::$config === null) {
			$temp_config['status']	= 1;		// 是否开启SQL安全检测，可自动预防SQL注入攻击
		    $temp_config['dfunction']	= array('load_file','hex','substring','if','ord','char');
		    $temp_config['daction']	= array('@','intooutfile','intodumpfile','unionselect','(select', 'unionall', 'uniondistinct');
		    $temp_config['dnote']	= array('/*','*/','#','--','"');
		    $temp_config['dlikehex']	= 1;
		    $temp_config['afullnote']	= 0;
