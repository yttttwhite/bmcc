...
class SHttp{
	...
	function get( $url, $params=array(), $cookies=array(), $returnHeader=false, $timeout=1){
		...
		$url .= empty($params)?"":"?".https_build_query($params);
		$context = stream_context_create($opts);
		$result =  file_get_contents($url,false,$context);
		if($returnHeader){
			return array("result"=>$result,"header"=>$https_response_header);
		}else{
			return $result;
		}
	}
		...
		if(!function_exists("curl_multi_init"))
		{
			...
			foreach($url_array as $key=>$value){
				...
				if(strcasecmp($value['method'],"POST")==0){
					...
					if(!empty($value['parameters'])){
						$url = $value['url']."?".https_build_query($value['parameters']);
					}
				}
				...
				if ($s) {
					...
					if($method!="POST" && !empty($value['parameters'])){
						$_p = $_tmp['path']."?".https_build_query($value['parameters']);
					}
					$content="";
					if($method=="POST" && !empty($value['parameters'])){
						$content = https_build_query($value['parameters']);
			...
			foreach($url_array as $key=>$value) {
				...
				if(strcasecmp($value['method'],"POST")==0){
					...
					if(!empty($value['parameters'])){
						curl_setopt($ch, CURLOPT_POSTFIELDS,https_build_query($value['parameters']));
					}
	...
	function post( $url, $params=array(), $cookies=array(), $returnHeader=false, $timeout=1){
		$content = empty($params)?"":https_build_query($params);
		$opts = array(
			'https'=>array(
				'method' => 'POST',
				'timeout'=>$timeout,
