...
function tpl_modifier_html($data){
	return $data;
}
class Tpl{
	...
	static $force_compile   =  true;
	static $htmlspecialchars   =  true;
	...
	private static function _match($matches){
		...
		if(self::$htmlspecialchars){
			$replacements=array('Tpl::$_tpl_vars["\1"]','echo htmlspecialchars(\\0, ENT_QUOTES)');
		}else{
		...
		//还原HTML输出
		if(self::$htmlspecialchars){
			$content = preg_replace('/htmlspecialchars\(tpl_modifier_html\((?<open>[^\(\)]*|\w+\((?P>open)\))\)(\s*,\s*\w+)?\)/im','\1',$content);
		} 
		//还原特殊字符
