...
PHP_METHOD(slightphp, run)
{
		...
		if (ZEND_NUM_ARGS()>0 && zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "z/", &path) != FAILURE) {
			...
			if(s==0){
				...
				if(zend_hash_find(Z_ARRVAL_P(PG(https_globals)[TRACK_VARS_GET]), 
							"PATH_INFO", sizeof("PATH_INFO"), (void **) &token) == SUCCESS
				){
					path = *token;
				}else if(zend_hash_find(Z_ARRVAL_P(PG(https_globals)[TRACK_VARS_SERVER]), 
							"PATH_INFO", sizeof("PATH_INFO"), (void **) &token) == SUCCESS
				){
					path = *token;
				}
...
PHP_MINFO_FUNCTION(slightphp)
{
		...
		php_info_print_table_row(2, "Supports", "https://code.google.com/p/slightphp/" );
